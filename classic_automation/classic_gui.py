...
class ClassicController:
    ...
    def _quad_crops(self, title_sub: str) -> dict:
        ...
        внешний rect и пропорции, замеренные по скриншоту 1375×994 px:
          PX1/PX2 — левый/правый край панелей (доля от ширины окна)
          PY1/PY2 — верхний/нижний край панелей (доля от высоты окна)
        Пропорции стабильны при любом DPI, если CLASSiC открыт в том же размере.
        """
        rect = _get_win_rect(title_sub)
        if not rect:
            return {}
        ...
        # Пропорции замерены по скриншоту 1375×994 @ 125% DPI
        x1 = x + int(w * 0.024)   # ≈ 33px  — левый край панелей
        x2 = x + int(w * 0.751)   # ≈ 1033px — правый край панелей
        y1 = y + int(h * 0.141)   # ≈ 140px  — верх панелей
        y2 = y + int(h * 0.649)   # ≈ 645px  — низ панелей
        mx = (x1 + x2) // 2
        my = (y1 + y2) // 2
