...
class ClassicController:
    ...
    def _quad_crops(self, title_sub: str) -> dict:
        """
        Режет окно на 4 квадранта (Характеристики).
        header=50px — заголовок окна + строка с кнопкой «Общий вид».
        """
        rect = _get_win_rect(title_sub)
        if not rect:
            return {}
        ...
        header = 50
        mx = x + w // 2
        my = y + header + (h - header) // 2
        return {
            'tl': (x,  y + header, mx,    my   ),  # Корневая плоскость
            'tr': (mx, y + header, x + w, my   ),  # Переходные процессы
            'bl': (x,  my,         mx,    y + h),  # Частотные характеристики
            'br': (mx, my,         x + w, y + h),  # Передаточные функции
        }
